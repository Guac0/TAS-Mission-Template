...
TAS_rallypointLocations pushBack [_rallypointPosATL,"Recon Rallypoint"];
		...
		while {_rallyStillExists} do {
			...
			if (_nearEnemiesNumber > _nearFriendliesNumberWeighted) then {
				...
				if (_overrunActive) then {
					private _msg = format ["The Recon Rallypoint at grid reference %1 has been overrun!", mapGridPosition _rallypointPosATL];
					_msg remoteExec ["hint"];

					{_x setDamage 1} forEach TAS_rallypointRecon;
					//TAS_rallypointReconRespawn call BIS_fnc_removeRespawnPosition;
					private _path = [TAS_rallypointLocations, "Recon Rallypoint"] call BIS_fnc_findNestedElement;
					private _indexOfOldRallyPair = _path select 0;
					TAS_rallypointLocations deleteAt _indexOfOldRallyPair;
				} else {
					private _msg = format ["The Recon Rallypoint at grid reference %1 is no longer in immediate danger of being overrrun!", mapGridPosition _rallypointPosATL];
					_msg remoteExec ["hint"];
				};
			};
