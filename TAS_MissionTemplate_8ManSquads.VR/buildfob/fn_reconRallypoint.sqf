...
if (TAS_rallyReconUsed == true) then {
	...
	private _path = [TAS_respawnLocations, "Recon Rallypoint"] call BIS_fnc_findNestedElement;
...
TAS_respawnLocations pushBack [_rallypointPosATL,"Recon Rallypoint"];
publicVariable "TAS_respawnLocations";
"rallypointReconMarker" setMarkerPosLocal getPos player; //updates the rallypoint's position on map
...
"rallypointReconMarker" setMarkerColor _color;	//last marker command is public
...
[player, format ["Recon rallypoint established by %1 at gridref %2.", name player, mapGridPosition player]] remoteExec ["sideChat", _friendlySide]; //tell everyone on same side about it
...
if (TAS_rallypointOverrun) then {
	[_friendlySide,_enemySides,_radius,_nearEnemies,_nearFriendlies,_nearEnemiesNumber,_nearFriendliesNumber,_rallypointPosATL] remoteExec ["TAS_fnc_ReconRallyOverrun",2]; //run overrun on server to avoid issues with FPS or client disconnect
};
