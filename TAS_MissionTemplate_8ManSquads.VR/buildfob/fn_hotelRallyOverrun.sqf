...
while {_rallyStillExists} do {
	...
	if (_nearEnemiesNumber > _nearFriendliesNumberWeighted) then {
		...
		if (_overrunActive) then {
			private _msg = format ["The Hotel Rallypoint at grid reference %1 has been overrun!", mapGridPosition _rallypointPosATL];
			_msg remoteExec ["hint"];
			...
			private _msg = format ["The Hotel Rallypoint at grid reference %1 is no longer in immediate danger of being overrrun!", mapGridPosition _rallypointPosATL];
			_msg remoteExec ["hint"];
		};
	};
