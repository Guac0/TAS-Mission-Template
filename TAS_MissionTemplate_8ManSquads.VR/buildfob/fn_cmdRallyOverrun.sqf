...
while {_rallyStillExists} do {
	...
	if ( ( _nearEnemiesNumber >= _minimumEnemies) && { _nearEnemiesNumber > _nearFriendliesNumberWeighted } ) then {
		...
		if (_overrunActive) then {
			private _msg = format ["The Cmd Rallypoint at grid reference %1 has been overrun!", mapGridPosition _rallypointPosATL];
			_msg remoteExec ["hint"];
			...
			"rallypointCmdMarker" setMarkerAlpha 0;
