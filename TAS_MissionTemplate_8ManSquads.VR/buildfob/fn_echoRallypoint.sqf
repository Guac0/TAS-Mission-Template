		...
		while {_rallyStillExists} do {
			...
			if (_nearEnemiesNumber > _nearFriendliesNumberWeighted) then {
				...
				if (_overrunActive) then {
					private _msg = format ["The Echo Rallypoint at grid reference %1 has been overrun!", mapGridPosition _rallypointPosATL];
					_msg remoteExec ["hint"];

					{_x setDamage 1} forEach TAS_rallypointEcho;
					//TAS_rallypointEchoRespawn call BIS_fnc_removeRespawnPosition;
					private _path = [TAS_rallypointLocations, "Echo Rallypoint"] call BIS_fnc_findNestedElement;
					private _indexOfOldRallyPair = _path select 0;
					TAS_rallypointLocations deleteAt _indexOfOldRallyPair;
				} else {
					private _msg = format ["The Echo Rallypoint at grid reference %1 is no longer in immediate danger of being overrrun!", mapGridPosition _rallypointPosATL];
					_msg remoteExec ["hint"];
				};
			};
