...
"rallypointEchoMarker" setMarkerPosLocal getPos player; //updates the rallypoint's position on map
...
"rallypointEchoMarker" setMarkerColor _color;	//last marker command is public
...
[player, format ["Echo rallypoint established by %1 at gridref %2.", name player, mapGridPosition player]] remoteExec ["sideChat", _friendlySide]; //tell everyone on same side about it
...
if (TAS_rallypointOverrun) then {
	[_friendlySide,_enemySides,_radius,_nearEnemies,_nearFriendlies,_nearEnemiesNumber,_nearFriendliesNumber,_rallypointPosATL] remoteExec ["TAS_fnc_EchoRallyOverrun",2]; //run overrun on server to avoid issues with FPS or client disconnect
};
