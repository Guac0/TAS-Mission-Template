...
if (TAS_rallyCharlieUsed == true) then {
	{deleteVehicle _x} forEach TAS_rallypointCharlie;
	//TAS_rallypointCharlieRespawn call BIS_fnc_removeRespawnPosition;
	private _path = [TAS_rallypointLocations, "Charlie Rallypoint"] call BIS_fnc_findNestedElement;
...
TAS_rallypointLocations pushBack [_rallypointPosATL,"Charlie Rallypoint"];
...
[player, format ["Charlie rallypoint established by %1 at gridref %2.", name player, mapGridPosition player]] remoteExec ["sideChat", _friendlySide]; //tell everyone on same side about it
		...
		while {_rallyStillExists} do {
			...
			if (_nearEnemiesNumber > _nearFriendliesNumberWeighted) then {
				...
				if (_overrunActive) then {
					private _msg = format ["The Charlie Rallypoint at grid reference %1 has been overrun!", mapGridPosition _rallypointPosATL];
					_msg remoteExec ["hint"];

					{_x setDamage 1} forEach TAS_rallypointCharlie;
					//TAS_rallypointCharlieRespawn call BIS_fnc_removeRespawnPosition;
					private _path = [TAS_rallypointLocations, "Charlie Rallypoint"] call BIS_fnc_findNestedElement;
					private _indexOfOldRallyPair = _path select 0;
					TAS_rallypointLocations deleteAt _indexOfOldRallyPair;
				} else {
					private _msg = format ["The Charlie Rallypoint at grid reference %1 is no longer in immediate danger of being overrrun!", mapGridPosition _rallypointPosATL];
					_msg remoteExec ["hint"];
				};
			};
