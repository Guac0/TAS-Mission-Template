...
TAS_respawnLocations pushBack [_rallypointPosATL,"Alpha Rallypoint"];
publicVariable "TAS_respawnLocations";
"rallypointAlphaMarker" setMarkerPosLocal getPos player; //updates the rallypoint's position on map
...
"rallypointAlphaMarker" setMarkerColor _color;	//last marker command is public
...
[player, format ["Alpha rallypoint established by %1 at gridref %2.", name player, mapGridPosition player]] remoteExec ["sideChat", _friendlySide]; //tell everyone on same side about it
...
if (TAS_rallypointOverrun) then {
	[_friendlySide,_enemySides,_radius,_nearEnemies,_nearFriendlies,_nearEnemiesNumber,_nearFriendliesNumber,_rallypointPosATL] remoteExec ["TAS_fnc_AlphaRallyOverrun",2]; //run overrun on server to avoid issues with FPS or client disconnect
};
