...
[player, format ["Foxtrot rallypoint established by %1 at gridref %2.", name player, mapGridPosition player]] remoteExec ["sideChat", _friendlySide]; //tell everyone on same side about it
		...
		while {_rallyStillExists} do {
			...
			if (_nearEnemiesNumber > _nearFriendliesNumberWeighted) then {
				...
				if (_overrunActive) then {
					private _msg = format ["The Foxtrot Rallypoint at grid reference %1 has been overrun!", mapGridPosition _rallypointPosATL];
					_msg remoteExec ["hint"];

					{_x setDamage 1} forEach TAS_rallypointFoxtrot;
					//TAS_rallypointFoxtrotRespawn call BIS_fnc_removeRespawnPosition;
					private _path = [TAS_rallypointLocations, "Foxtrot Rallypoint"] call BIS_fnc_findNestedElement;
					private _indexOfOldRallyPair = _path select 0;
					TAS_rallypointLocations deleteAt _indexOfOldRallyPair;
