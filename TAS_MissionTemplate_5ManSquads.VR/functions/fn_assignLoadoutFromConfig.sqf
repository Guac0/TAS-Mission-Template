...
if (_givenUnit getVariable ["TAS_overrideConfigLoadout",false]) then {
	_roleDescription = _givenUnit getVariable ["TAS_overrideConfigLoadout",false];
} else {
...
switch (count _matchingUnitArray) do {
	case 0: { //no results, so first try to basic rifleman, then take any unit
		...
		systemChat "Role description did not match a unit type in the given faction, fallback loadout assigned!";
	};
