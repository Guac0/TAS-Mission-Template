...
if (TAS_rallyAlphaUsed == true) then { {deleteVehicle _x} forEach TAS_rallypointAlpha; TAS_rallypointAlphaRespawn call BIS_fnc_removeRespawnPosition;}; //if rallypoint already exists, delete it so the new one can be spawned
TAS_rallypointAlphaRespawn = [side player, getPos player, "Alpha Rallypoint"] call BIS_fnc_addRespawnPosition;

if (TAS_useSmallRally == false) then {
	TAS_rallypointAlpha = [getPos player, getDir player, call (compile (preprocessFileLineNumbers "buildfob\rallypointComposition.sqf"))] call BIS_fnc_ObjectsMapper;
} else {
...
publicVariable "TAS_rallyAlphaUsed"; //might be unneccessary