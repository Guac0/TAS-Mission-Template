...
TAS_rallypointCmdRespawn = [side player, getPos player, "Cmd Rallypoint"] call BIS_fnc_addRespawnPosition;

if (TAS_useSmallRally == false) then {
	TAS_rallypointCmd = [getPos player, getDir player, call (compile (preprocessFileLineNumbers "buildfob\rallypointComposition.sqf"))] call BIS_fnc_ObjectsMapper;
} else {
...
publicVariable "TAS_rallyCmdUsed"; //might be unneccessary